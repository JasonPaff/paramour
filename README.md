# paramour

> *paramour* — "param" + "amour": a library that loves your params.

A fully type-safe routing companion for the Next.js App Router: validated, typed
route params and search params, type-checked path building, and a predictable,
human-readable URL wire format — with your choice of validation library via
[Standard Schema](https://github.com/standard-schema/standard-schema).

## Status

**This is a name-reservation placeholder.** The library is under active design
and development — nothing here is usable yet.

- Design document and progress: [github.com/JasonPaff/paramour](https://github.com/JasonPaff/paramour)
- Companion packages will live under the [`@paramour-js`](https://www.npmjs.com/org/paramour-js) scope.

## Why

Born from lessons learned maintaining and contributing to
[`next-typesafe-url`](https://www.npmjs.com/package/next-typesafe-url), which is
effectively unmaintained. Paramour aims to be a documented, tested,
community-maintained answer to typed routing in Next.js, with a migration path
from `next-typesafe-url`.

## License

MIT © Jason Paff
