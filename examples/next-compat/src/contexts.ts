/**
 * Pins core's structural `PagesContext` (from `route.ts`) against real Next.
 *
...
/**
 * `getStaticProps` deliberately does NOT compose: its context has no `query`
 * — at build time there is no query string, and typed search there would be
 * a lie. If Next ever ADDS `query` to `GetStaticPropsContext`, this
 * expectation fails and that stance needs re-deciding, which is the point of
 * pinning it.
 */
// @ts-expect-error -- GetStaticPropsContext has no query member.
