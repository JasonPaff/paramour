...
// ("href arguments"). Since the string overload landed, failing route-object
// calls report TS2769 anchored at the call — the specific diagnosis lives in
// the "last overload gave the following error" detail.
...
// String form. This example registers "/" and "/product/[id]" as app
// routes — so "/" is the only registered STATIC path.

// @expect-error TS2769 — unregistered string: the world-A fallback is gone
href("/about");

// @expect-error TS2769 — a dynamic path string needs a route object
href("/product/[id]");

// @expect-error TS2769 — the string form is hash-only: no raw-search side door
href("/", { search: { q: "x" } });
