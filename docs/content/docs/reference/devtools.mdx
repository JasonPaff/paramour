...
The public surface is deliberately tiny — the plugin-entry helper and the
panel component — because the panel never re-implements decoding: it
subscribes to an observation seam the hooks emit through, so what it renders
is what your components actually got.
