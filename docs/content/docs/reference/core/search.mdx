...
The exact wire behavior these helpers implement — absence vs. the empty
string, ordering, duplicate keys, elision — is the
[wire-format spec](/docs/reference/wire-format)'s numbered rule set; this
page documents the function contracts.
...
// Optional third argument: anchor a thrown SearchDecodeError to a route.
decodeSearch(config, { page: "x" }, "/products");
```

The third argument, `routePath`, sets a thrown `SearchDecodeError`'s
`route` field and its message header (`Failed to decode search params for
/products:`). The route methods pass `route.path` automatically; a bare
`decodeSearch` call without it throws a route-less error.
