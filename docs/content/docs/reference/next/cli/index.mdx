---
title: paramour CLI
description: Invocation, configuration precedence, and the exit-code contract shared by all six commands.
---
...
| [`skills`](/docs/reference/next/cli/skills)     | Install or verify the bundled agent skill for detected agent tools.   |
...
| `1`  | The thing you asked to verify is not true — `check` / `generate --check` drift, any failed `doctor` check, `skills --check` on missing or stale installed skills. Nothing else. |
