/**
 * Type-level tests for the devtools seam contract — the seam types exercised
 * from the next package's suite. The devtools package consumes these same
 * types via the types-only `@paramour-js/next/devtools-seam` subpath, so what's
 * pinned here is the cross-package contract.
 */
...
test("status narrows the result three ways", () => {
...
test("ParamourHookId is exactly the six hook literals (Pages has no OrThrow)", () => {
...
test("a concretely-typed result still fits: the shape is generic-erased", () => {
