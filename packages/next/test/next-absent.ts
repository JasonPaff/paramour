 ...
 * real-Next fallback adapters — imports Vite must RESOLVE to load those
 * modules, even though no test may ever call through them: every hook suite
 * supplies its framework reads via the `@paramour-js/next/testing` provider
 * (or a hand-built adapter on the seam context). So this shim carries no
 * state and no `__set*` choreography — every export throws, which IS the
 * assertion that the real-Next fallback path stays unreached under test.
 *
