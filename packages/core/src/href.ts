 ...
 * it is design-02 D4's rule surfacing at the property level. `hash`
 * implements S10 — fragments come only from an explicit caller option.
 */
 ...
 * `object extends` probe).
 */
type PartFor<Key extends string, Input> =
 ...
 * many routes. All errors are `SerializeError` at link-build time; a JS
...
export function href<R extends AnyRoute>(
  ...
  // The conditional tuple is unresolved inside the generic body; this cast
  // unifies its two branches (same move as defineRoute's config cast).
  const [options] = args as [InferHrefInput<AnyRoute>?];
